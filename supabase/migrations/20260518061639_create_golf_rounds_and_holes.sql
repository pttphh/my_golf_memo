/*
  ...
  - Putting: putt_count, putt_memo

  ## Security
  - RLS enabled on both tables
  - Public read/write policies for all (no auth requirement for this app)
*/
...
CREATE POLICY "Anyone can select rounds"
...
CREATE TABLE IF NOT EXISTS holes (
  ...
  putt_count int NOT NULL DEFAULT 0,
  putt_memo text DEFAULT ''
);

ALTER TABLE holes ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can select holes"
  ON holes FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY "Anyone can insert holes"
  ON holes FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

CREATE POLICY "Anyone can update holes"
  ON holes FOR UPDATE
  TO anon, authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Anyone can delete holes"
  ON holes FOR DELETE
  TO anon, authenticated
  USING (true);
