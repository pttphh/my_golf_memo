import { Home, BarChart3, Flag, User } from 'lucide-react';
...
const TABS: { id: NavTab; label: string; icon: typeof Home }[] = [
  { id: 'round-list', label: '라운드', icon: Home },
  { id: 'all-rounds', label: '통계', icon: BarChart3 },
  { id: 'new-round', label: '라운드 설정', icon: Flag },
  { id: 'profile', label: '개인 정보', icon: User },
];
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
      <div className="w-full max-w-[390px] px-4 pointer-events-auto">
        <nav className="bg-white rounded-full shadow-lg border border-gray-100 px-2 py-2 flex items-center justify-around">
          {TABS.map(tab => {
            const Icon = tab.icon;
