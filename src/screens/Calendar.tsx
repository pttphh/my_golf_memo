...
const MEMO_KEY = 'golf_cell_memos';
...
export default function Calendar({ onRoundSelect }: Props) {
  ...
  const [memoEdit, setMemoEdit] = useState<{ key: string; text: string } | null>(null);
  ...
  function startPress(d: Date, e: React.TouchEvent | React.MouseEvent) {
    ...
    longPressTimer.current = window.setTimeout(() => {
      ...
      setMemoEdit({ key: k, text: memos[k] ?? '' });
    }, 500);
  }
  ...
  function saveMemo() {
    ...
    setMemos(prev => {
      const next = { ...prev };
      if (text) next[memoEdit.key] = text;
            ...
            {cells.map(d => {
              ...
              return (
                <button
                  ...
                  className={`min-h-[72px] rounded-lg flex flex-col items-center pt-1 px-0.5 overflow-hidden active:scale-95 transition-transform select-none ${isToday ? 'ring-2 ring-[#1B4332]' : ''} ${inMonth ? '' : 'opacity-45'}`}
                ...
                >
                  ...
                  {planState === 'temp' && (
                    <span className="mt-1.5 w-2.5 h-2.5 rounded-full border-2" style={{ borderColor: level >= 4 ? '#FFFFFF' : '#1B4332' }} />
                  )}
                  {planState === 'confirmed' && (
                    <Flag size={13} className="mt-1.5" strokeWidth={2.5} style={{ color: level >= 4 ? '#FFFFFF' : '#1B4332' }} />
                  )}
                  {!hasRound && memo && (
                    <span className="mt-auto mb-1 text-[9px] font-semibold leading-tight text-center w-[90%] truncate px-1 py-0.5 rounded" style={{ backgroundColor: 'rgba(255,255,255,0.9)', color: '#1B4332' }}>
                      {memo}
                    </span>
                  )}
                </button>
...
{memoEdit && (
        ...
        <div className="absolute inset-0 bg-black/40" onPointerDown={() => setMemoEdit(null)} />
          <div className="relative bg-white rounded-t-2xl w-full max-w-[390px] p-5 pb-10">
            ...
            <textarea
              autoFocus
              value={memoEdit.text}
              onChange={e => setMemoEdit(prev => (prev ? { ...prev, text: e.target.value } : prev))}
